...
class LANSyncServer:
    ...
    def __init__(self, project_id, project_manager, password_hash, on_remote_update, password=None):
        self.project_id = project_id
        self.project_manager = project_manager
        self.password_hash = password_hash
        self.on_remote_update = on_remote_update
        # The host process knows the plaintext password at share-start time
        # (typed into the New Project / Start Sharing form); we keep it only
        # in memory so we can derive the same Fernet key clients use.
