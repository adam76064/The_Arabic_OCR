...
import fitz  # PyMuPDF
import numpy as np
from table_detector import orchestrator, line_grid_detector
...
class TableHandler:
    ...
    def process_table_layout(self, project_id, page_index, block_index, extraction_method="auto"):
        ...
        if not project: return {'ok': False}
        ...
        # ──────────────────────────────────────────────────────────
        # TIER 1: Native PDF Vectors (المتجهات الرقمية الأصلية)
        # ──────────────────────────────────────────────────────────
        if extraction_method in ["auto", "native"] and pdf_path and os.path.exists(pdf_path):
            import fitz
            from table_detector import line_grid_detector
            try:
        ...
        # ──────────────────────────────────────────────────────────
        # TIER 2: Coordinates (إحداثيات الكلمات من OCR بدون لمس الصورة)
        # ──────────────────────────────────────────────────────────
        if grid is None and extraction_method in ["auto", "coordinates"]:
            ...
            if blobs:
                from table_detector import column_finder, blob_detector, row_reconciler, row_grouper
        ...
        # ──────────────────────────────────────────────────────────
        # TIER 3: Smear / Image Morphology (معالجة الفراغات والخطوط بصرياً)
        # ──────────────────────────────────────────────────────────
        ...
        # بناء الخلايا النهائية للـ JSON
        # بناء الخلايا النهائية للـ JSON
        cols_x, rows_y = sorted(list(set(cols_x))), sorted(list(set(rows_y)))
        cells_72dpi = []
        
        # 1. تحميل النسخة الخام الأصلية للأداء المضمون للأحرف والكلمات
        from backend.text_retriever import extract_text_for_bbox
        raw_ocr_blocks = self.project_manager.load_raw_ocr(project_id, page_index)

        # 2. بناء مصفوفة الخلايا واستخراج نصوصها من البيانات الخام بدون تشويه
        all_table_text = []
        ...
        # 3. تحديث الكتلة بهيكلية الجدول الجديدة والنص المجمع
        ...
        # تحديث النص الرئيسي للكتلة ليعكس محتوى الجدول
        table_block['text'] = "\n".join(all_table_text)
