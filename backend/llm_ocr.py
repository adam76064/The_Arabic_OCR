...
class LLMOCRHandler:
    ...
    def extract_page(self, image_path: str, llm_config: dict) -> dict:
        """
        يرسل الصورة للنموذج المختار ويرجع كائن JSON المهيكل.
        """
        if litellm is None:
        ...
        # تهيئة المتغيرات لـ litellm
        model_id = provider
        api_base = None

        # توجيه مفتاح الـ API للشركة الصحيحة بناءً على الـ prefix
        if provider == 'custom':
            ...
            # Fallback for Qwen, Llama, GLM (عادة ما يتم استدعاؤهم عبر OpenRouter أو TogetherAI أو خوادم متوافقة مع OpenAI)
            os.environ["OPENAI_API_KEY"] = api_key
        ...
        try:
            ...
            # تنظيف الـ JSON من الـ Markdown fences
