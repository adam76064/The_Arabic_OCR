...
import json
import re
import difflib # 🪄 مكتبة بايثون السحرية لمقارنة تشابه الحروف (مفيدة جداً للـ OCR)
...
class QuranHandler:
    ...
    def _load_data(self):
        if not os.path.exists(self.json_path):
            print(f"⚠️ Warning: Quran.json not found at {self.json_path}")
            return
    ...
    def _normalize_text(self, text):
        ...
        # 2. توحيد أشكال الحروف (يجب أن يتم أولاً لإنقاذ ألف الوصل من الحذف!)
        text = re.sub(r'[أإآٱ]', 'ا', text)
        text = re.sub(r'[ىئ]', 'ي', text)
        text = re.sub(r'ة', 'ه', text)
        text = re.sub(r'ؤ', 'و', text)
        
        # 3. القائمة البيضاء: إزالة كل شيء عدا الحروف العربية الأساسية والمسافات
    ...
    def search_text(self, query):
        ...
        # ---------------------------------------------------------
        # 1. التطابق التام (Exact Substring Match) - أعلى أولوية
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # 2. النوافذ المتداخلة (للجمل الطويلة >= 3 كلمات)
        # ---------------------------------------------------------
        if length >= 3:
        ...
        # ---------------------------------------------------------
        # 3. مقارنة الحروف السحرية (للكلمات القصيرة جداً 1 أو 2 كلمة)
        # يحل مشكلة الـ OCR في الآيات ذات الكلمة الواحدة مثل (مدهامتان)
        # ---------------------------------------------------------
        else:
            for entry in self.search_corpus:
                clean_text = self._normalize_text(entry['ayah']['text'])
                
                # إذا كانت الآية بأكملها تشبه البحث بنسبة 75% على الأقل
                if difflib.SequenceMatcher(None, clean_query, clean_text).ratio() > 0.75:
                    scores[entry['id']] = scores.get(entry['id'], 0) + 200
                else:
                    # أو إذا كان البحث يشبه أي كلمة داخل الآية بنسبة 80%
        ...
        # استخراج أفضل 30 نتيجة وترتيبها مؤقتاً بتسلسل المصحف
        top_entries = sorted(
        ...
        # ---------------------------------------------------------
        # 4. ملء الفجوات الذكي (Smart Gap Fill)
        # ---------------------------------------------------------
                ...
                if curr_ayah['surah_number'] == next_ayah['surah_number']:
                    ...
                    if 1 < gap <= 5: 
                        for missing_num in range(curr_ayah['ayah_number'] + 1, next_ayah['ayah_number']):
                            missing_ayah = next((a['ayah'] for a in self.search_corpus if a['surah_number'] == curr_ayah['surah_number'] and a['ayah_number'] == missing_num), None)
                            if missing_ayah and missing_ayah not in filled_results:
                                filled_results.append(missing_ayah)
                                # توريث النقاط للآية المفقودة لكي تبقى مرتبطة بمجموعتها
        ...
        # ---------------------------------------------------------
        # 5. المعالجة البعدية (Post-Processing): الكتل والترتيب النهائي!
        # ---------------------------------------------------------
        if not results: return []
        
        # أ. تجميع الآيات المتتالية في كتل (Blocks)
        blocks = []
        ...
        # ب. ترتيب الكتل بناءً على أعلى تقييم (Score) حصلت عليه أي آية داخلها
        blocks.sort(key=lambda block: max(scores.get(ayah['id'], 0) for ayah in block), reverse=True)

        # ج. فك الكتل وإعادتها كقائمة واحدة ليقرأها المتصفح
        final_results = []
        for block in blocks:
