...
class PaddleOCRClient:
    ...
    # ==========================================
    def get_limits(self):
        """Returns the number of trials left for today."""
    ...
    def decrement_limit(self):
        """Decreases the daily limit by 1."""
    ...
    def _generate_bce_headers(self, ak, sk, token, bucket, object_key, method="PUT", query_params=None):
        ...
        if query_params:
            query_parts = [f"{urllib.parse.quote(k, safe='')}={urllib.parse.quote(str(v), safe='')}" if v != "" else f"{urllib.parse.quote(k, safe='')}=" for k, v in sorted(query_params.items())]
    ...
    def process_pdf_chunk(self, file_path, window=None):
        """Uploads a PDF chunk, polls for completion, and returns the parsed JSON data."""
        ...
        part_headers = self._generate_bce_headers(ak, sk, token, bucket, object_key, method="PUT", query_params={"partNumber": "1", "uploadId": upload_id})
        ...
        # 3. Create Task
        task_res = session.post("https://aistudio.baidu.com/paddlex/v3/ocr/tasks", json={"fileNames": [safe_api_filename], "parseModel": "PaddleOCR-VL-1.6"})
        task_data = task_res.json().get("result", [])
        task_id = task_data[0].get("taskId") if isinstance(task_data, list) else task_data.get("taskId")
        
        session.get("https://aistudio.baidu.com/paddlex/v3/ocr/tasks/addfile", params={
            "source": "paddleOcr", "fileOriginName": safe_api_filename, "taskId": task_id, "bucketName": bucket, "fileKey": object_key
        })
    ...
    def parse_paddle_to_app_format(self, paddle_pages, project_pages, start_idx):
        """Converts PaddleOCR's JSON output into the app's standard 200 DPI block format."""
        app_pages = []
        for i, page_data in enumerate(paddle_pages):
            actual_page_index = start_idx + i
            
            # Avoid index out of bounds if Paddle returns extra pages
            if actual_page_index >= len(project_pages):
                break
                
            target_page = project_pages[actual_page_index]
            
            # The frontend (review.js) expects all coordinates to be scaled to 200 DPI.
            # We calculate what the width/height SHOULD be in 200 DPI space:
            ...
            json_w = pruned.get("width", 1)  # Paddle's internal canvas width
            json_h = pruned.get("height", 1) # Paddle's internal canvas height
            ...
            for item in items:
                ...
                category = "Text"
                if "table" in label.lower(): category = "Table"
                elif "image" in label.lower() or "figure" in label.lower(): category = "Picture"
                elif "title" in label.lower() or "header" in label.lower(): category = "Title"
