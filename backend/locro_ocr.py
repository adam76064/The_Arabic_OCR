...
def get_screen_ai():
    ...
    try:
        # Use system-installed Chrome's ScreenAI model from default path
        return ScreenAI()
    except Exception as e:
        raise RuntimeError(f"تعذر تشغيل نموذج Locro ScreenAI: متصفح Google Chrome غير مثبت في المسار الافتراضي على الجهاز أو غير متاح.\nالتفاصيل: {e}")
...
def run_locro_ocr(image_path, config=None):
    ...
    # Locro returns pages -> blocks -> lines -> words
    for page in data.get('pages', []):
        ...
        for b_idx, block in enumerate(page.get('blocks', [])):
            ...
            # Map block_type to our categories if possible
            category = 'Text'
            if block_type == 'table': category = 'Table'
            elif block_type == 'image': category = 'Picture'
            elif block_type == 'list': category = 'List-item'
            ...
            for line in block.get('lines', []):
                ...
                for word in line.get('words', []):
                    ...
                    w_entry = {
                        ...
                        "bbox": w_bbox,
                        "geometry": w_geom,
                        "confidence": round(word.get('confidence', 1.0), 4)
                    }
                ...
                # Expand block bbox to encompass all lines if block doesn't have a direct bbox
